...
*/
    ...
    eote.process.setup = function(cmd, playerName, playerID){
        ...
        if (unusableMatch) {
            ...
            sendChat(diceObj.vars.characterName, 'Votre arme est trop endommagée pour être utilisée. Essayez de la réparer.');
            return false;
        }
    ...
    eote.process.rollPlayer = function(cmd, diceObj){
        ...
        if (charcterMatch) {
            ...
            if (charObj.length > 0){
                ...
                sendChat("Alert", "Impossible de trouver le PJ. Mettez à jour le nom du PJ correspondant à sa fiche et réessayer, svp.");
                return false;
            }
        } else {
            sendChat("Alert", "Mettez à jour le nom du PJ, svp.");
            return false;
        }
    ...
    eote.process.label = function(cmd, diceObj){
        ...
        if (label) {
			var labelStr = '<table style="border:2px solid black; border-collapse:collapse; width:100%; padding:2px;">';
            //var labelStr = '<table style="border:2px solid black; border-collapse:collapse; padding:2px; width:100%;">';
            ...
            _.each(labelArray, function(labelVal){
                ...
                labelStr = labelStr + '<tr><td style="border-bottom:1px solid black; border-collapse:collapse; padding:2px;"><b>' + label + ':</b></td>';
				labelStr = labelStr + '<td style="border-bottom:1px solid black; border-collapse:collapse; padding:2px;"> ' + message + '</td></tr>';
            });
			
            //labelStr = labelStr + '</table>';
    ...
    eote.process.crit = function(cmd, diceObj){
        ...
        var critTable = [
            {
        		...
        		severity : 1,
        		name : 'Broutille',
        		Result : 'Vous subissez un point de stress permanent.',
        		effect : '',
        	},
        	{
        		percent : '6 to 10',
        		severity : 1,
        		name : 'Ralenti',
        		Result : 'Ne peut agir lors de la dernière place d`Initiative alliée sur le prochain tour.',
        		effect : '',
        	},
        	{
        		percent : '11 to 15',
        		severity : 1,
        		name : 'Brusque secousse',
        		Result : 'Vous laissez tomber ce que vous avez dans les mains.',
        		effect : '',
        	},
        	{
        		percent : '16 to 20',
        		severity : 1,
        		name : 'Distrait',
        		Result : 'Vous ne pouvez pas effectuer de manoeuvre gratuite au prochain tour.',
        		effect : '',
        	},
        	{
        		percent : '21 to 25',
        		severity : 1,
        		name : 'Déséquilibré',
        		Result : 'Ajoutez un dé noir au prochain test de carac.',
        		effect : '',
        	},
        	{
        		percent : '26 to 30',
        		severity : 1,
        		name : 'Blessure décourageante',
        		Result : 'Vous perdez un point de destin.',
        		effect : '',
        	},
        	{
        		percent : '31 to 35',
        		severity : 1,
        		name : 'Etourdi',
        		Result : 'Chancellant, vous ne pouvez pas faire d`action au prochain tour.',
        		effect : '',
        	},
        	{
        		percent : '36 to 40',
        		severity : 1,
        		name : 'Piqûre',
        		Result : 'Augmente la difficulté du prochain test de carac (1 dé violet).',
        		effect : '',
        	},
        	//----------------------------- Severity 2
        	{
        		...
        		severity : 2,
        		name : 'Bouleversé',
        		Result : 'Mis à terre et vous subissez 1 point de stress permanent.',
        		effect : '',
        	},
        	{
        		percent : '46 to 50',
        ...
        var critRoll = function(addCritNum) {
            ...
            if (!openSlot) {
                sendChat("Alert", "Pourquoi vous n'êtes pas mort !");
        	    return false;
            }
        	...
        	for (var key in critTable)  {
    			...
    			if ((rollTotal >= low) && (rollTotal <= high)) {
                    ...
                    /*var chat = '/direct <br><b>Blessure critiques</b><br>';
					...
					var chat = '/direct <table style="border:2px solid black; border-collapse:collapse;">';
						chat = chat + '<tr><td style="background-color:black; font-weight:bold; color:white; text-align:center;">Blessure critiques</td></tr>';
						chat = chat + '<tr><td style="border:2px solid black; border-collapse:collapse; text-align:center;"><img src="http://i.imgur.com/z51hRwd.png" /></td></tr>';
						chat = chat + '<tr><td>Blessures actuelles: (' + totalcrits + ' x 10)</td></tr>';
						if (rollOffset) {
							chat = chat + '<tr><td>Compensation: ' + rollOffset + '</td></tr>';
						}
						chat = chat + '<tr><td>Résultat des dés: ' + diceRoll + '</td></tr>';
						chat = chat + '<tr><td>Total: ' + rollTotal + '</td></tr>';
						chat = chat + '<tr><td></td></tr>';
						chat = chat + '<tr><td style="border:2px solid black; border-collapse:collapse; text-align:center; color:white; background-color:#444444;"><b>' + critTable[key].name + '</b></td></tr>';
						chat = chat + '<tr><td style="border:2px solid black; border-collapse:collapse;">' + critTable[key].Result + '</td></tr>';
						chat = chat + '</table>';
        ...
        var critRoll = function(addCritNum) {
        	...
        	for (var key in critTable)  {
    			...
    			if ((rollTotal >= low) && (rollTotal <= high)) {
                    ...
                    var chat = '/direct <table style="border:2px solid black; border-collapse:collapse;">';
						chat = chat + '<tr><td style="background-color:black; font-weight:bold; color:white; text-align:center;">Dégâts critiques</td></tr>';
						chat = chat + '<tr><td style="border:2px solid black; border-collapse:collapse; text-align:center;"><img src="http://i.imgur.com/JO3pOr8.png" /></td></tr>';//need new graphic
						chat = chat + '<tr><td>Dégâts actuelles: (' + totalcrits + ' x 10)</td></tr>';
						if (rollOffset) {
							chat = chat + '<tr><td>Compensation: ' + rollOffset + '</td></tr>';
						}
						chat = chat + '<tr><td>Résultat des dés: ' + diceRoll + '</td></tr>';
    ...
    eote.process.diceOutput = function(diceObj, playerName, playerID) {
        ...
        if (diceObj.totals.success > 0) {
        	diceTextResults = diceTextResults + " Succès:" + diceObj.totals.success;
    	...
    	if (diceObj.totals.failure > 0) {
    		diceTextResults = diceTextResults + " Echec:" + diceObj.totals.failure;
    		for (i=1;i<=diceObj.totals.failure;i++){
    			diceGraphicsResults = diceGraphicsResults + s1 + eote.defaults.graphics.SYMBOLS.F + s2 + "Echec" + s3 + eote.defaults.globalVars.diceGraphicsChatSize + s4 + eote.defaults.globalVars.diceGraphicsChatSize + s5;
    		}
    	}
    	if (diceObj.totals.advantage > 0) {
    		diceTextResults = diceTextResults + " Avantage:" + diceObj.totals.advantage;
    	...
    	if (diceObj.totals.threat > 0) {
    		diceTextResults = diceTextResults + " Menace:" + diceObj.totals.threat;
    		for (i=1;i<=diceObj.totals.threat;i++){
    			diceGraphicsResults = diceGraphicsResults + s1 + eote.defaults.graphics.SYMBOLS.T + s2 + "Menace" + s3 + eote.defaults.globalVars.diceGraphicsChatSize + s4 + eote.defaults.globalVars.diceGraphicsChatSize + s5;
    		}
    	}
    	if (diceObj.totals.triumph > 0) {
    		diceTextResults = diceTextResults + " Triomphe:" + diceObj.totals.triumph;
    		for (i=1;i<=diceObj.totals.triumph;i++){
    			diceGraphicsResults = diceGraphicsResults + s1 + eote.defaults.graphics.SYMBOLS.TRIUMPH + s2 + "Triomphe" + s3 + eote.defaults.globalVars.diceGraphicsChatSize + s4 + eote.defaults.globalVars.diceGraphicsChatSize + s5;
    		}
    	}
    	if (diceObj.totals.despair > 0) {
    		diceTextResults = diceTextResults + " Désastre:" + diceObj.totals.despair;
    	...
    	if (diceObj.totals.light > 0) {
    		diceTextResults = diceTextResults + " Lumière:" + diceObj.totals.light;
    		
            for (i=1; i<=diceObj.totals.light; i++){
    			diceGraphicsResults = diceGraphicsResults + s1 + eote.defaults.graphics.SYMBOLS.L + s2 + "Lumière" + s3 + eote.defaults.globalVars.diceGraphicsChatSize + s4 + eote.defaults.globalVars.diceGraphicsChatSize + s5;
    		}
    	}
    	if (diceObj.totals.dark > 0) {
    		diceTextResults = diceTextResults + " Obscure:" + diceObj.totals.dark;
    		for (i=1;i<=diceObj.totals.dark;i++){
    			diceGraphicsResults = diceGraphicsResults + s1 + eote.defaults.graphics.SYMBOLS.D + s2 + "Obscure" + s3 + eote.defaults.globalVars.diceGraphicsChatSize + s4 + eote.defaults.globalVars.diceGraphicsChatSize + s5;
    		}
    	}
        ...
        //------------------------------------>
		//chatGlobal = chatGlobal + '<table style="border:2px solid black; border-collapse:collapse; width:100%; padding:2px;">';
		chatGlobal = chatGlobal + '<tr><td colspan="2" style="background-color:black; font-weight:bold; color:white; text-align:center;">Jet</td></tr>';
        ...
        if (eote.defaults.globalVars.diceLogChat === true) {
        	if (eote.defaults.globalVars.diceLogRolledOnOneLine === true) {
                ...
                if (diceObj.textLog.Boost !="") diceTextRolled = diceTextRolled + "Fortune:"+diceObj.textLog.Boost;
    			if (diceObj.textLog.Ability !="") diceTextRolled = diceTextRolled + "Abilité:"+diceObj.textLog.Ability;
    			if (diceObj.textLog.Proficiency !="") diceTextRolled = diceTextRolled + "Maitrise:"+diceObj.textLog.Proficiency;
    			if (diceObj.textLog.SetBack !="") diceTextRolled = diceTextRolled + "Infortune:"+diceObj.textLog.SetBack;
    			if (diceObj.textLog.Difficulty !="") diceTextRolled = diceTextRolled + "Difficulté:"+diceObj.textLog.Difficulty;
    			if (diceObj.textLog.Challenge !="") diceTextRolled = diceTextRolled + "Défi:"+diceObj.textLog.Challenge;
    			if (diceObj.textLog.Force !="") diceTextRolled = diceTextRolled + "Force:"+diceObj.textLog.Force;
				if (diceObj.textLog.Success !="") diceTextRolled = diceTextRolled + "Succès:"+diceObj.textLog.Success;
				if (diceObj.textLog.Advantage !="") diceTextRolled = diceTextRolled + "Advantage:"+diceObj.textLog.Advantage;
                if (diceObj.textLog.Failure != "") diceTextRolled = diceGraphicsRolled + "Echec:"+diceObj.textLog.Failure;
                if (diceObj.textLog.Threat != "") diceTextRolled = diceGraphicsRolled + "Menace:"+diceObj.textLog.Threat;
    
    			if (eote.defaults.globalVars.diceGraphicsChat === true) {	
					
					chatGlobal = chatGlobal + '<tr><td colspan="2" style="border:2px solid black; border-collapse:collapse; padding:2px;">';
                    chatGlobal = chatGlobal + diceGraphicsRolled;
					chatGlobal = chatGlobal + '</td></tr>';
    			} else {
                ...
                if (eote.defaults.globalVars.diceGraphicsChat === true) {
                    ...
                    if (diceObj.vars.label) {
                        sendChat(characterPlayer, "/direct <br><b>Comp.:</b> " + diceObj.vars.label + '<br>');
                    }
    				...
    				if (diceObj.textLog.Boost !="") sendChat("", "Fortune:"+diceObj.textLog.Boost);
    				if (diceObj.textLog.Ability !="") sendChat("", "Abilité:"+diceObj.textLog.Ability);
    				if (diceObj.textLog.Proficiency !="") sendChat("", "Maitrise:"+diceObj.textLog.Proficiency);
    				if (diceObj.textLog.SetBack !="") sendChat("", "Infortune:"+diceObj.textLog.SetBack);
    				if (diceObj.textLog.Difficulty !="") sendChat("", "Difficulté:"+diceObj.textLog.Difficulty);
    				if (diceObj.textLog.Challenge !="") sendChat("", "Défi:"+diceObj.textLog.Challenge);
    				if (diceObj.textLog.Force !="") sendChat("", "Force:"+diceObj.textLog.Force);
					if (diceObj.textLog.Success !="") sendChat("", "Succès:"+diceObj.textLog.Success);
					if (diceObj.textLog.Advantage !="") sendChat("", "Advantage:"+diceObj.textLog.Advantage);
					if (diceObj.textLog.Failure !="") sendChat("", "Echec:" + diceObj.textLog.Failure);    
                    if (diceObj.textLog.Threat !="") sendChat("", "Menace:" + diceObj.textLog.Threat);
        ...
        if (eote.defaults.globalVars.diceGraphicsChat === true ) {
			...
			chatGlobal = chatGlobal + '<tr><td colspan="2" style="border:2px solid black; border-collapse:collapse; text-align:center; color:white; background-color:#444444; padding:2px;">';
			chatGlobal = chatGlobal + 'Résultat';
			chatGlobal = chatGlobal + '</td></tr>';
			
			if(successOrFail == "fail"){
				chatGlobal = chatGlobal + '<tr><td colspan="2" style="border-bottom:1px solid black; border-collapse:collapse; text-align:center; color:white; background-color:#B40404; padding:2px;">';
				chatGlobal = chatGlobal + 'Fail';
				chatGlobal = chatGlobal + '</td></tr>';
			}
			if(successOrFail == "success"){
				chatGlobal = chatGlobal + '<tr><td colspan="2" style="border-bottom:1px solid black; border-collapse:collapse; text-align:center; color:white; background-color:#04B404; padding:2px;">';
				chatGlobal = chatGlobal + 'Success';
				chatGlobal = chatGlobal + '</td></tr>';
			}
			
			chatGlobal = chatGlobal + '<tr><td colspan="2" style="border:2px solid black; border-collapse:collapse; padding:2px;">';
			chatGlobal = chatGlobal + diceGraphicsResults;
			chatGlobal = chatGlobal + '</td></tr>';
			...
			if (diceObj.totals.success > 0 && diceObj.vars.weaponDamage && diceObj.vars.weaponCritical){
				...
				chatGlobal = chatGlobal + '<tr><td colspan="2" style="border:2px solid black; border-collapse:collapse; text-align:center; color:white; background-color:#444444; padding:2px;">';
				chatGlobal = chatGlobal + 'Damages';
				chatGlobal = chatGlobal + '</td></tr>';
				chatGlobal = chatGlobal + '<tr><td colspan="2" style="border:2px solid black; border-collapse:collapse; padding:2px; text-align:center; font-weight:bold;">';
				chatGlobal = chatGlobal + damage;
				chatGlobal = chatGlobal + '</td></tr>';
								
				if (diceObj.totals.advantage >= diceObj.vars.weaponCritical){
					
					chatGlobal = chatGlobal + '<tr><td colspan="2" style="border:2px solid black; border-collapse:collapse; text-align:center; background-color:darkred; color:white; font-weight:bold; padding:2px;">';
					chatGlobal = chatGlobal + 'CRITICAL HIT !';
					chatGlobal = chatGlobal + '</td></tr>';
				}
			}
			
			chatGlobal = chatGlobal + '</table>';
